...
# forward-reference (e.g. User.default_workspace: Mapped["Workspace | None"])
# is resolved. Mapper configuration runs lazily, on first real ORM use — a
# model whose module nothing in the app's actual runtime import graph ever
# imports raises "failed to locate a name" at that point, even though the
# class itself is defined correctly. Same reasoning as alembic/env.py's
# identical import list, applied here so the app itself doesn't depend on
# that migration-only entry point.
